...
ra.bookings.defaults = function (defaults) {
    ...
    this.closingdate = defaults.closingdate;
    ...
    this.display = function (tag) {
        ...
        switch (this.closingdate) {
            case '6pm':
                this.addLi(elements.closing, "6pm on the day before event");
                break;
            case '24':
                this.addLi(elements.closing, "24hours before event start");
                break;
            case 'start':
                this.addLi(elements.closing, "event start time");
                break;
            default:
                this.addLi(elements.closing, "INVALID");
        }
        if (this.waitinglist) {
    ...
    this.overrides = function (options) {
        ...
        if (this.closingdate !== options.closingdate) {
            switch (options.closingdate) {
                case '6pm':
                    out[out.length] = "Booking closing: 6pm on the day before event";
                    break;
                case '24':
                    out[out.length] = "Booking closing: 24hours before event start";
                    break;
                case 'start':
                    out[out.length] = "Booking closing: event start time";
                    break;
            }
        }
...
// Display summary table row
    ...
    this.displayUserInfo = function (tag, userId, closing) {
        if (closing.closed()) {
            ra.bookings.addTextTag(tag, 'h3', "Sorry, booking is now closed");
            return;
        } else {
            ra.bookings.addTextTag(tag, 'div', "Booking closes at " + closing.toString());
        }
        if (userId === 0) {
            if (this.options.guest) {
...
ra.bookings.closing = function (ev, evb) {
    this.settingPeriod;
    this.datetime;
    this._closed = false;

    // calculate start time
    var period = evb.options.closingdate;
    var walkdate = ev.getEventDatetime();
    var closingDate = new Date(walkdate.getTime());

    switch (period) {
        case '6pm':
            closingDate.setDate(closingDate.getDate() - 1);  // Subtract one day
        ...
        case 'start':
            break;
        default:
            var hours = parseInt(period, 10);
            if (!isNaN(hours) && Number.isInteger(hours)) {
                closingDate.setHours(closingDate.getHours() - hours);
            }
    }
    this.datetime = closingDate;
    ...
    this.toString = function () {
        return  ra.time.HHMM(this.datetime) + " " + ra.date.dowddmmyyyy(this.datetime);
    };
};